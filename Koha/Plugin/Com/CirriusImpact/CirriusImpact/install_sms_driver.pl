...
This script installs the SMS::Send::CirriusImpact driver to the plugins/SMS/Send
directory, which is already in Perl's @INC path. This driver is required for 
the CirriusImpact plugin to integrate with Koha's message queue system.

The script does not require root privileges since it installs to the plugins
directory.
...
# Find the plugins SMS/Send directory
sub find_plugins_sms_dir {
    my $plugins_dir = find_plugins_dir();
    return "$plugins_dir/SMS/Send";
}
...
sub install_drivers {
    my ($us_source, $intl_source, $plugins_sms_dir) = @_;
    
    print_info "Installing SMS::Send drivers to plugins directory...";
    
    # Install US::CirriusImpact (main driver)
    my $us_dest_dir = "$plugins_sms_dir/US";
    ...
    my $intl_dest_dir = "$plugins_sms_dir";
...
sub main {
    ...
    my $plugins_sms_dir = find_plugins_sms_dir();
    
    # Check if already installed
    my $us_dest = "$plugins_sms_dir/US/CirriusImpact.pm";
    my $intl_dest = "$plugins_sms_dir/CirriusImpact.pm";
    ...
    # Install both drivers
    install_drivers($us_source, $intl_source, $plugins_sms_dir);
