...
sub _generate_csv_output {
    ...
    # messageText added at the end for message content
    my @headers = qw(
        ...
        PickupAreaDescription TxnID AccountBalance messageText
    );
    ...
    for my $message (@$message_data) {
        ...
        # Add message text based on transport type
        my $message_text = '';
        if ($transport eq 'sms') {
    ...
    $data->{sms}->{text} = $self->_ci_insert_title_into_text($fallback, $title);
		...
		eval { $self->_ci_backfill_additional_identifiers($data) };
            ...
            my %mf = (
                ...
                STAB_userSaluation => defined $pat->{title}               ? $pat->{title}               : '',
        ...
        if ($using_whatsapp) {
            ...
            delete $data->{sms};
                ...
                # commType mapping: V=phone, E=email, S=sms
                my $commType  = $transport eq 'phone' ? 'V' :
                                $transport eq 'email' ? 'E' :
                                $transport eq 'sms'   ? 'S' : '';
                ...
                $mf->{STAB_userSaluation} = defined $pat->{title}               ? $pat->{title}               : '';
...
for my $k (keys %$mf) {
    ...
    my $csv_data = _generate_csv_output(\@filtered_message_data);
...
sub _ci_postfill_sms {
    ...
    $fill->($s, 'STAB_userSaluation',    $get->($patron,'title',''));
